...
export function processArgsOptions () {
  ...
  return { programArgs, programOptions }
}

export async function cli (workspaceFilePath, options) {
  const workspace = await Workspace.load(workspaceFilePath, options)
  await workspace.run()
}
